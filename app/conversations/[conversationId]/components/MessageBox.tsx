    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <mark key={i} className="bg-yellow-700 text-yellow-100 px-0.5">
            {part}
          </mark>
        ) : (
          part
...
const MessageBox = ({
  ...
  const seenList = (data.seen || [])
    .filter(
      (user) =>
        user.email !== data?.sender?.email && user.email !== currentUserEmail,
    )
    .map((user) => user.name)
    .join(", ");

  // 이모지별 카운트 그룹핑
  const reactionGroups = (data.reactions ?? []).reduce<
  ...
  return (
    <div className={clsx("flex gap-3 px-4 pt-2 pb-1", isOwn && "justify-end")}>
      {/* 상대방 아바타 */}
      {!isOwn && (
      ...
      >
        ...
        {data.replyTo && !isDeleted && (
          ...
          >
            ...
            <span className="ml-1">
              {data.replyTo.image ? "📷 이미지" : data.replyTo.body}
            </span>
          </div>
        )}
        ...
        {/* 메시지 버블 + 액션 버튼 */}
        <div
          className={`group/bubble flex flex-col ${isOwn ? "items-end" : ""}`}
        >
          {/* 버블 */}
          {isDeleted ? (
            <div className="text-sm text-gray-600 italic px-2.5 py-1.5 border border-dashed border-[#2e2e2e]">
              삭제된 메시지입니다.
            </div>
          ) : (
            <div
        ...
        <div className="flex items-center gap-1">
          <span className="text-xs text-gray-600">{data.sender.name}</span>
